...
def draw_from_dict(dicdata,RANGE):
    ...
    plt.title("objdump中变异算子被杀死的比例", fontsize=15, pad=20)
...
if __name__ == "__main__":
    ...
    with open('../../mull_result/objdump_result', encoding='utf-8') as file_obj:
    ...
    for i in range(len(contents)):
        if(contents[i].find("warning: Killed")!=-1):
            temp=contents[i]
            key = contents[i][temp.find('[')+1:temp.find(']')]
    ...
    for i in range(len(contents)):
        if (contents[i].find("warning: Survived") != -1):
            temp = contents[i]
            key = contents[i][temp.find('[') + 1:temp.find(']')]
    ...
    for i in (killed_mutant_dateDict).keys():
        if(survived_mutant_dateDict.get(i,0)==0):
            ...
            ratioDict[i]=killed_mutant_dateDict[i]/survived_mutant_dateDict[i]
